...
export default function BulkTranslateEpisodesButton() {
  ...
  async function run() {
    setState('running')
    setResult(null)
    const res = await fetch('/api/admin/translate/bulk-episodes', { method: 'POST' })
  ...
  return (
    <div className="flex flex-col gap-1">
      <button onClick={run} disabled={state === 'running'} className="btn-secondary text-sm">
        {state === 'running' ? 'Načítám...' : 'TMDB překlady popisů epizod'}
      </button>
      {result && (
